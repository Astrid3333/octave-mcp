...
# ---------------------------------------------------------------------------
# Dispatcher para tool_registry
# ---------------------------------------------------------------------------
def _dispatch(operation, **kwargs):
    """Despacha a la función correspondiente según operation."""
    fn_map = {
    ...
    if operation not in fn_map:
        raise ValueError(f"operation desconocida: {operation}. Válidas: {list(fn_map.keys())}")
    return fn_map[operation](**kwargs)
...
TOOL_SCHEMA = {
    "name": TOOL_NAME,
    "description": (
        "Química de iones en solución: conversión de concentraciones, fuerza "
        "iónica, coeficiente de actividad (Davies), cálculo de pH (Henderson-"
        "Hasselbalch, ácido débil exacto, capacidad buffer), producto de "
        "solubilidad (Ksp, efecto de ion común, precipitación), conductividad "
        "iónica (Kohlrausch), coeficiente de difusión (Nernst-Einstein, Stokes-"
        "Einstein), movilidad electroforética (Hückel, Smoluchowski) y "
    ...
    "inputSchema": {
        ...
        "properties": {
            ...
            "V1": {"type": "number", "description": "Volumen inicial."},
            "C2": {"type": "number", "description": "Concentración final."},
            "V2": {"type": "number", "description": "Volumen final."},
            "mass_g": {"type": "number", "description": "Masa de soluto (g)."},
            "molar_mass_g_mol": {"type": "number", "description": "Masa molar (g/mol)."},
            "molality_mol_kg": {"type": "number", "description": "Molalidad (mol/kg)."},
            "density_kg_L": {"type": "number", "description": "Densidad (kg/L)."},
            "ppm": {"type": "number", "description": "Concentración en ppm."},
            "species": {"type": "array", "description": "Lista de especies iónicas.", "items": {"type": "object"}},
            "z": {"type": "number", "description": "Carga iónica."},
            "ionic_strength_mol_L": {"type": "number", "description": "Fuerza iónica (mol/L)."},
            "T_K": {"type": "number", "description": "Temperatura en Kelvin (default 298.15)."},
            "model": {"type": "string", "description": "Modelo a usar (default davies)."},
            "a_ion_nm": {"type": "number", "description": "Tamaño iónico (nm)."},
            "solvent": {"type": "string", "description": "Solvente (default water)."},
            "pKa": {"type": "number", "description": "pKa del ácido."},
            "conc_base": {"type": "number", "description": "[A-]."},
            "conc_acid": {"type": "number", "description": "[HA]."},
            "H_conc_mol_L": {"type": "number", "description": "Concentración de H+ (mol/L)."},
            "gamma": {"type": "number", "description": "Coeficiente de actividad de H+ (default 1.0)."},
            "Ka": {"type": "number", "description": "Constante de acidez."},
            "C_total_mol_L": {"type": "number", "description": "Concentración analítica total (mol/L)."},
            "Kw": {"type": "number", "description": "Producto iónico del agua (default 1e-14)."},
            "solubility_mol_L": {"type": "number", "description": "Solubilidad s (mol/L)."},
            "p": {"type": "integer", "description": "Coeficiente estequiométrico del catión (default 1)."},
            "q": {"type": "integer", "description": "Coeficiente estequiométrico del anión (default 1)."},
            "Ksp": {"type": "number", "description": "Producto de solubilidad."},
            "common_ion_conc_mol_L": {"type": "number", "description": "Concentración del ion común (mol/L)."},
            "common_is_cation": {"type": "boolean", "description": "Si el ion común es el catión (default False)."},
            "cation_conc_mol_L": {"type": "number", "description": "Concentración del catión (mol/L)."},
            "anion_conc_mol_L": {"type": "number", "description": "Concentración del anión (mol/L)."},
            "cation": {"type": "string", "description": "Catión."},
            "anion": {"type": "string", "description": "Anión."},
            "nu_cation": {"type": "integer", "description": "Subíndice estequiométrico del catión (default 1)."},
            "nu_anion": {"type": "integer", "description": "Subíndice estequiométrico del anión (default 1)."},
            "kappa_S_per_m": {"type": "number", "description": "Conductividad específica (S/m)."},
            "C_mol_per_m3": {"type": "number", "description": "Concentración (mol/m³)."},
            "lambda_m0": {"type": "number", "description": "Conductividad molar límite."},
            "K_coef": {"type": "number", "description": "Coeficiente K de Kohlrausch-Onsager."},
            "C_mol_L": {"type": "number", "description": "Concentración (mol/L)."},
            "mobility_m2_Vs": {"type": "number", "description": "Movilidad iónica (m²/(V·s))."},
            "viscosity_Pa_s": {"type": "number", "description": "Viscosidad del solvente (Pa·s)."},
            "radius_m": {"type": "number", "description": "Radio hidrodinámico (m)."},
            "lambda0_S_cm2_mol": {"type": "number", "description": "Conductividad molar límite (S·cm²/mol)."},
            "D_m2_s": {"type": "number", "description": "Coeficiente de difusión (m²/s)."},
            "drift_velocity_m_s": {"type": "number", "description": "Velocidad de deriva (m/s)."},
            "E_field_V_m": {"type": "number", "description": "Campo eléctrico aplicado (V/m)."},
            "eps_r": {"type": "number", "description": "Permitividad relativa (default 78.30)."},
            "zeta_potential_V": {"type": "number", "description": "Potencial zeta (V)."},
            "C_salt_out": {"type": "number", "description": "Concentración de sal externa (mol/L)."},
            "C_fixed_charge_in": {"type": "number", "description": "Concentración de carga fija (mol/L)."},
            "z_fixed": {"type": "integer", "description": "Carga del macroión (default -1)."},
            "z_salt": {"type": "integer", "description": "Carga del electrolito (default 1)."},
